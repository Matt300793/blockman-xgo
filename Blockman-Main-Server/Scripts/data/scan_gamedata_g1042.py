...
table_now = 'prod_g1042'
table_old_829 = 'g1042_8_29'
...
def write_to_csv(fn, data):
    p = os.path.join(CUR_DIR, fn)
    with open(p, 'w+') as f:
        f.write('user_id,game_id,props_id,count,yaoshi_829,yaoshi_now,money_829,money_now\n')
        for row in data:
            f.write('%s,%s,%s,%s,%s,%s,%s,%s\n' % (
                row['user_id'], row['game_id'], row['props_id'], row['count'],row.get('YaoShi_829', '0'),row.get('YaoShi_now', '0'),row.get('Currency_829', '0'),row.get('Currency_now', '0')))
...
def query_user_yaoshi():
    ...
    rows = read_csv('user_g1042.csv')
    for row in rows:
        print(row)
        current = query_game_data(table_now, '1', row['user_id'])
        old = query_game_data(table_old_829, '1', row['user_id'])

        row = job_times(old, row, '_829')
        row = job_times(current, row, '_now')
        results.append(row)

    write_to_csv('user_g1042-results.csv', results)
