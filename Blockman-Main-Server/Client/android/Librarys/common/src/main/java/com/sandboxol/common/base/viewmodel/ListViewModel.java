...
public abstract class ListViewModel<T> extends ViewModel implements IListViewModel<T> {
    ...
    public final ItemViewSelector<ListItemViewModel<T>> itemView = new BaseItemViewSelector<ListItemViewModel<T>>() {
        @Override
        public void select(ItemView itemView, int position, ListItemViewModel<T> item) {
    ...
    public void removeItem(T item) {
        Observable.from(itemViewModel)
                ...
                .subscribe(itemViewModel::remove);
    }
    ...
    @Override
    public void bindView(ItemView itemView, int position, ListItemViewModel item) {
        if (model != null) {
    ...
    public void onDestroy() {
        if (model != null) {
            if (model.getRemoveToken() != null) {
                Messenger.getDefault().unregister(this, model.getRemoveToken());
            }
            if (model.getInsertToken() != null) {
                Messenger.getDefault().unregister(this, model.getInsertToken());
            }
            if (model.getReplaceToken() != null) {
                Messenger.getDefault().unregister(this, model.getReplaceToken());
            }
            if (model.getRefreshToken() != null) {
                Messenger.getDefault().unregister(this, model.getRefreshToken());
            }
            Observable.from(itemViewModel).subscribe(ViewModel::onDestroy);
        }
    }
