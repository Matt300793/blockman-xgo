 ...
 */
public class EditorMapListModel extends PageListModel<McMap> {

    public EditorMapListModel(Context context, int errorResId) {
        super(context, errorResId);
    }

    @Override
    public void onBind(ItemView itemView, int position, ListItemViewModel<McMap> item) {
        itemView.set(BR.ViewModel, R.layout.item_editor_map);
    }
    ...
    public ListItemViewModel<McMap> getItemViewModel(McMap item) {
        return new EditorMapItemViewModel(context, item);
    }
